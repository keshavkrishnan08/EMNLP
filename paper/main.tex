...
interpretable parameters. The floor at zero exposure, \Ezero, is the
quantity we care about most: a direct, per-construction measure of how much
of a construction is learnable from \emph{indirect} evidence alone, turning
the filtered-corpus debate into a number. The half-saturation dose \Efifty{}
and the steepness \hilln{} then say how much direct evidence the rest takes
and whether it arrives smoothly or at a threshold. We use a tiered design to
keep this affordable: eight constructions get \Ezero{} (zero-dose), four of
them get the full dose ladder, and one shared full-corpus model per seed
supplies the ceiling --- 63 models in a single frozen recipe.
...
across all runs. Tuning per dose would just invite worries about
p-hacking; freezing the recipe is what keeps the comparison honest.

We deliberately avoid a full factorial. The floor \Ezero{} --- our headline
--- needs only the zero-dose model plus a ceiling, so we measure it
\emph{broadly}: eight constructions are trained at zero exposure. Four
\emph{core} constructions also get the full dose ladder, so we can fit the
curve and read off \Efifty{} and \hilln; these span the space from
productive-but-rare AANN through the semi-idiomatic comparative correlative
\citep{weissweiler2022comparative}, the poverty-of-stimulus case of
tough-movement \citep{hu2020syntaxgym}, to the argument-structure resultative
\citep{goldberg2004resultatives}. The other four (existential \emph{there},
it-clefts, negative inversion, and ECM raising) broaden the range of
\Ezero{} we observe. One shared full-corpus model per seed supplies the
ceiling for everyone --- at dose ``all'' the corpus is just the unfiltered
corpus, so training it once rather than per construction is what buys the
breadth. Three seeds per cell make seed spread a real distribution. The whole
design is 63 models.
...
language models have reopened \citep{lan2026llmsps}. The core targets here
...
results. The recipe is frozen across all runs.
...
\resultpending{quantify and contrast \Efifty{} across the core constructions}
...
a frozen 63-model tiered sweep with matched-replacement dose corpora --- isolates
